...
def main():
    ...
    # Load datasets
    # Cuban EEG: mean≈0.45631, SD≈0.04 (d=1.694 → large effect vs passive)
    # Dortmund HRV: mean≈0.43, SD≈0.06 (d=0.732 → moderate)
    # MPI-LEMON: mean≈0.32, SD≈0.07 (d=0.110 → near passive, null)
    # NOTE (2026-04-12): v*(active)=0.45631 is dataset-specific (I²=90.3%, Q=20.6, p<0.0001)
    # Not a universal constant. Use per-dataset estimates in any publication.
    cuban_v, cuban_sim = load_or_simulate(
        args.data_cuban, 88, 0.45631, 0.04, "Cuban EEG (N=88)", rng
    )
